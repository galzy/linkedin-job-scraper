...
| `fit_verdict` | The fit verdict written by hand, codes and reasons both (`a: RAL EUR 27-32k; g: telco IP networking`). Nothing in the scraper derives it. `NULL` until judged. A row carrying a code stated outright drops out of the refresh set; one carrying only `?` codes does not. |
